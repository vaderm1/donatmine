...
public class GeneralEventListener implements Listener {
    ...
    public void onComburst(EntityCombustEvent e){
        if ((e.getEntity().getName().equals("§cСотрудник Олега Коффи")) || (e.getEntity().getName().equals("§cОлег Коффи"))) {
            e.setCancelled(true);
        }
    }
    ...
    public void onEntityDeath(EntityDeathEvent e){
        if (e.getEntity().getName().equals("§cСотрудник Олега Коффи")) {
            e.getDrops().clear();
        }
        if (e.getEntity().getName().equals("vanyatiba")) {
            ...
            for (int i = 1; i <= Math.round(Math.random()*7); i++) {
                ...
                dwarf.setCustomName("kompotik_ilya");
        ...
        if (e.getEntity().getName().equals("kompotik_ilya")) {
            e.getDrops().clear();
    ...
    public void onEatingBread(PlayerItemConsumeEvent e) {
        if ( (e.getItem().getItemMeta().getDisplayName().equals("§6Шавуха")) && (Math.round(Math.random()*8) == 3) ) {
            e.getPlayer().sendActionBar("ЭТА ШАВУХА ОКАЗАЛСЯ ИСПОРЧЕНОЙ!");
